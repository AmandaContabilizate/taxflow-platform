...
/** Regularizaciones (kind 1) compradas y en proceso. Sin `rfc` trae las de todos. */
export async function getRegularizationsByTaxpayer(
  ...
  const { rfc, skip, take, kind, taxRegimeId, statusId } = parsed.data

  try {
    const data = await fetchGet<PagedDeclarations<TaxpayerDeclarationItem>>(
      API_ROUTES.DECLARATION.REGULARIZATIONS_BY_TAXPAYER({ rfc, skip, take, kind, taxRegimeId, statusId }),
      'declaration',
    )
    ...
    console.error('[getRegularizationsByTaxpayer] Error:', e)
    return err({ statusCode: 500, message: 'No pudimos obtener las regularizaciones.' })
  }
}
